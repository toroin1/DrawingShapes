...
{
    [Serializable]
    public class isoscelesTrig : Shape
    {
        ...
        public override void Paint(Graphics graphics, int thickness)
        {
            ...
            points[0] = new Point(Location.X + FrameConverter(FrameWidth) / 2, Location.Y);
        ...
        public override bool Intersect(Shape frame)
        {
            return
                Location.X < frame.Location.X + frame.FrameWidth && frame.Location.X < Location.X + FrameConverter(FrameWidth) &&
                Location.Y < frame.Location.Y + frame.FrameHeight && frame.Location.Y < Location.Y + FrameConverter(FrameHeight);
